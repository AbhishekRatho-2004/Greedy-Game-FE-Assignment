...
export default function AdminAddTodoDrawer({ todo, onClose, refreshTodos }) {
  ...
  useEffect(() => {
    if (todo) {
      ...
      setAssignedUser(todo.personId?._id || "");
    } else {
      setTitle("");
      setDescription("");
      setDueDate("");
  ...
  const handleSubmit = async (e) => {
    ...
    if (!title || !description || !dueDate || !dueTime || !assignedUser) {
    ...
    try {
      ...
      if (todo && todo._id) {
        ...
        res = await axios.post(
          `http://localhost:5000/api/admin/users/${assignedUser}/todos`,
          payload,
          {
            headers: { Authorization: `Bearer ${token}` },
            withCredentials: true,
  ...
  return (
    <div className="fixed top-0 right-0 w-[400px] h-full bg-white shadow-lg border-l z-50 transition-all">
      <div className="flex justify-between items-center p-4 border-b">
        <h2 className="text-lg font-semibold">{todo ? "Edit Todo" : "Assign Todo to User"}</h2>
